...
class WhatsAppWebhook(models.AbstractModel):
    ...
    _description = "WhatsApp Webhook Processing"

    @api.model
    def _enqueue_payload(self, backend, payload):
        """Queue one job per incoming message and per status update.

        Meta retries a webhook until it gets a 200 and redelivers for days,
        so the request only queues work: the job keyed by the WhatsApp id
        runs once, and a slow media download or chatbot reply never holds
        Meta's request.
        """
            ...
            for change in entry.get("changes", []):
                ...
                for status in value.get("statuses", []):
                    self.with_delay(
                        channel=JOB_CHANNEL,
                        identity_key=(
                            f"whatsapp-status-{backend.id}-{status.get('id')}"
                            f"-{status.get('status')}"
                        ),
                        description=f"WhatsApp: status of {status.get('id')}",
                        max_retries=STATUS_MAX_RETRIES,
                    )._process_message_status(backend, status)
    ...
    def _process_message_status(self, backend, status):
        ...
        if not message and self._is_recent(status):
            # The send that created the message may not be committed yet
            raise RetryableJobError(
                "WhatsApp message of this status not found yet",
                seconds=STATUS_RETRY_SECONDS,
            )
    ...
    def _process_incoming_message(self, backend, value, message, full_payload):
        self = self._with_backend_language(backend)
        backend = backend.with_env(self.env)
        message_model = self.env["whatsapp.message"].sudo()
        existing = message_model.search(
            [("backend_id", "=", backend.id), ("message_id", "=", message.get("id"))],
            limit=1,
        )
        if existing:
            # A redelivery: the message and any chatbot reply already exist
            return
    ...
    def _download_whatsapp_media(self, backend, media_id):
        ...
        try:
            ...
            response = requests.get(
                media_url, headers=headers, timeout=MEDIA_TIMEOUT_SECONDS
            )
            ...
            download_response = requests.get(
                download_url, headers=headers, timeout=MEDIA_TIMEOUT_SECONDS
            )
    ...
    def _resolve_chatbot_step(self, chatbot, thread, message_text):
        ...
        """
        script_model = self.env["whatsapp.chatbot.script"].sudo()
    ...
    def _with_backend_language(self, backend):
        """Answer in the backend's language (chatbot texts are translated)."""
        if backend.language:
            return self.with_context(lang=backend.language.code)
        return self

    @api.model
    def _is_recent(self, status):
        """Tell whether a status was sent in the last STATUS_RETRY_WINDOW."""
        timestamp = int(status.get("timestamp") or 0)
        return time.time() - timestamp < STATUS_RETRY_WINDOW_SECONDS
